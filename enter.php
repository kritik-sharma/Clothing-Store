        ...
        <?php
            if($_SERVER["REQUEST_METHOD"] == 'POST')
            {
                ...
                if($_POST["act"] == "signup")
                {
                    ...
                    if($username == "")
                    {
                        echo "<script>alert('Username can't be left empty!')</script>";
                        die();
                    }
                    if($first_name == "")
                    {
                        echo "<script>alert('First Name can't be left empty!')</script>";
                        die();
                    }
                    if($email == "")
                    {
                        echo "<script>alert('Email can't be left empty!')</script>";
                        die();
                    }
                    if($password == "")
                    {
                        echo "<script>alert('Password can't be left empty!')</script>";
                        die();
                    }
                    $password = password_hash($password,PASSWORD_DEFAULT);
                    if($confirm_password == "")
                    {
                        echo "<script>alert('Confirm Password can't be left empty!')</script>";
                        die();
                    }    
                    ...
                    $check = "SELECT * from userss where user_id='$username'";
                    if(mysqli_num_rows(mysqli_query($conn,$check)) != 0)
                    {
                        echo "<script>alert('Username already exists!')</script>";
                        die();
                    }
                    $check = "SELECT * from userss where email_id='$email'";
                    if(mysqli_num_rows(mysqli_query($conn,$check)) != 0)
                    {
                        echo "<script>alert('User with this Email ID exists!')</script>";
                        die();
                    }
                    $query = "INSERT INTO userss VALUES('$username','$email','$password','$first_name','$last_name','$mobile_no')";
                ...
                else if($_POST["act"] == "login")
                {
                    ...
                    $check = "SELECT * from userss where user_id='$username' LIMIT 1";
